...
## 模拟器（MUMU 等）「桥接模式 + root」能否替代真机？（调研结论）

**结论：不可行。** 具体如下：

### 桥接模式做了什么

MUMU 的「网络桥接模式」只是把模拟器的**虚拟网卡**桥接到宿主机的物理网卡，
让模拟器像局域网里一台独立设备一样直接拿到路由器的 IP（而非 NAT 内网）。
它解决的是「网络互通 / 抓包 / ADB」问题，**并没有给模拟器一个真实的 802.11 射频**。

### 为什么跑不了 WPS 攻击

| 需求 | 模拟器现实 |
|------|-----------|
| 真实 `wlan0` + 802.11 射频 | ❌ MUMU 里的 `wlan0` 是**虚拟接口**（virtio-net 之类），Wi-Fi 界面显示的是 "WiredSSID" |
| 监听模式 / 帧注入（Pixie Dust 抓 M1-M7 的底层） | ❌ 虚拟网卡是「以太网」语义，无 802.11 管理帧能力 |
| `wpa_supplicant` 驱动真实射频做 WPS 交换 | ❌ 模拟器里没有由 wpa_supplicant 管理的真实无线网卡 |

一句话：桥接只是「虚拟以太网」通了，跟「真无线射频」是两回事；root 再高也变不出射频。

### 真正可行的模拟器/虚拟机路线（需 USB 网卡直通）

只有把一块 **USB 无线网卡直通进虚拟机**、且安卓镜像带其驱动时，才可能获得真实 `wlan0`：

1. **Android-x86 / BlissOS** 装进 VirtualBox / VMware / QEMU，用 **USB 直通**挂载一块
   支持监听模式的 Atheros/Ralink 网卡（如 AR9271）。这是目前唯一「模拟器跑 WPS 攻击」的可行路径。
2. MUMU / 雷电 / 夜神等**游戏向模拟器不支持任意 USB 设备直通**，无法挂载真实网卡。

> 因此本仓库**不为 MUMU 桥接模式添加「兼容性」代码**——它从根本上不具备射频能力，加了也无意义。
> 请优先用真机 + Termux，或 Linux + USB 网卡（见 `../linux/README.md`）。
